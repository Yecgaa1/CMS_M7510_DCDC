...
void INTP_Start(uint16_t pinMsk)
{
    if(pinMsk & (1 << 0))
    {
        NVIC_ClearPendingIRQ(INTP0_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP0_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 1))
    {
        NVIC_ClearPendingIRQ(INTP1_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP1_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 2))
    {
        NVIC_ClearPendingIRQ(INTP2_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP2_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 3))
    {
        NVIC_ClearPendingIRQ(INTP3_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP3_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 4))
    {
        NVIC_ClearPendingIRQ(INTP4_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP4_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 5))
    {
        NVIC_ClearPendingIRQ(INTP5_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP5_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 6))
    {
        NVIC_ClearPendingIRQ(INTP6_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP6_IRQn);       /* enable INTP interrupt */
    }
    if(pinMsk & (1 << 7))
    {
        NVIC_ClearPendingIRQ(INTP7_IRQn); /* clear INTP interrupt flag */
        NVIC_EnableIRQ(INTP7_IRQn);       /* enable INTP interrupt */
    }
}
/***********************************************************************************************************************
...
void INTP_Stop(uint16_t pinMsk)
{
    if(pinMsk & (1 << 0))
    {
        NVIC_DisableIRQ(INTP0_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP0_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 1))
    {
        NVIC_DisableIRQ(INTP1_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP1_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 2))
    {
        NVIC_DisableIRQ(INTP2_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP2_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 3))
    {
        NVIC_DisableIRQ(INTP3_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP3_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 4))
    {
        NVIC_DisableIRQ(INTP4_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP4_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 5))
    {
        NVIC_DisableIRQ(INTP5_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP5_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 6))
    {
        NVIC_DisableIRQ(INTP6_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP6_IRQn);  /* clear INTP interrupt flag */
    }
    if(pinMsk & (1 << 7))
    {
        NVIC_DisableIRQ(INTP7_IRQn);       /* disable INTP interrupt */
        NVIC_ClearPendingIRQ(INTP7_IRQn);  /* clear INTP interrupt flag */
    }
}
