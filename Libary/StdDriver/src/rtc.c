...
{
    CGC->PERA  |= CGC_PERA_RTCEN_Msk;     /* enables input clock supply */
    if(rtccks == RTC_FSUB) {
...
{
    RTC->RTCC0  = _00_RTC_COUNTER_STOP;     /* stops counter operation */
    NVIC_DisableIRQ(RTC_IRQn);      /* disable INTRTC interrupt */
    NVIC_ClearPendingIRQ(RTC_IRQn); /* clear INTRTC interrupt flag */
    NVIC_ClearPendingIRQ(RTC_IRQn); /* clear INTRTC interrupt flag */
}
/***********************************************************************************************************************
...
MD_STATUS RTC_Set_ConstPeriodInterruptOn(rtc_int_period_t period)
{
    ...
    else
    {
        ...
        RTC->RTCC1 &= (uint8_t)~_08_RTC_NVIC_GENERATE_FLAG;
        NVIC_ClearPendingIRQ(RTC_IRQn); /* clear INTRTC interrupt flag */
        NVIC_ClearPendingIRQ(RTC_IRQn); /* clear INTRTC interrupt flag */
        NVIC_EnableIRQ(RTC_IRQn);       /* enable INTRTC interrupt */
    }
...
void IT_Init(rtc_cks_t rtccks, uint16_t counter_val)
{
    ...
    NVIC_DisableIRQ(IT_IRQn);       /* disable INTIT interrupt */
...
{
    NVIC_ClearPendingIRQ(IT_IRQn);      /* clear INTIT interrupt flag */
    NVIC_EnableIRQ(IT_IRQn);            /* enable INTIT interrupt */
