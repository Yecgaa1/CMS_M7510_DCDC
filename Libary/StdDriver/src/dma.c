               ...
               dma_size_t size, uint16_t count, void *src_adr, void *dst_adr)
{
    ...
    DMAVEC->CTRL[ctrl_data_num].DMACR = (size << DMAVEC_CTRL_DMACR_SZ_Pos)      | (1 << DMAVEC_CTRL_DMACR_RPTINT_Pos)    |
                                        (damod << DMAVEC_CTRL_DMACR_DAMOD_Pos)  | (samod << DMAVEC_CTRL_DMACR_SAMOD_Pos) |
                                        (rptsel << DMAVEC_CTRL_DMACR_RPTSEL_Pos)| (mode << DMAVEC_CTRL_DMACR_MODE_Pos);
    ...
    /* init DMA registers */
    CGC->PERA   |= CGC_PERA_DMAEN_Msk;
    DMA->DMABAR  = DMAVEC_BASE;
    sfr_addr = &DMA->DMASET0;
