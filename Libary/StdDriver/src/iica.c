...
{
    CGC->PERA |= CGC_PERA_IICA0EN_Msk;              /* enables input clock supply */
    IICA0->IICCTL00 = _00_IICA_OPERATION_STOP;       /* stop operation */
    NVIC_DisableIRQ(IICA0_IRQn);                         /* disable INTIICA interrupt flag */           
    NVIC_ClearPendingIRQ(IICA0_IRQn);                    /* clear INTIICA interrupt flag */         
...
condition */
    ...
    NVIC_EnableIRQ(IICA0_IRQn);                          /* enable INTIICA interrupt flag */     
...
MD_STATUS IICA0_MasterSend(uint8_t adr, uint8_t * const tx_buf, uint16_t tx_num, uint8_t wait)
{
    ...
    NVIC_DisableIRQ(IICA0_IRQn);   /* disable INTIICA0 interrupt */

    if (IICA0->IICF0 & IICA0_IICF0_IICBSY_Msk)
    {
        /* Check bus busy */
        NVIC_EnableIRQ(IICA0_IRQn);   /* enable INTIICA0 interrupt */
        status = MD_ERROR1;
    }
    else
    {
...
{
    CGC->PERA |= CGC_PERA_IICA1EN_Msk;              /* enables input clock supply */
    IICA1->IICCTL10 = _00_IICA_OPERATION_STOP;       /* stop operation */
    NVIC_DisableIRQ(IICA1_IRQn);                         /* disable INTIICA interrupt flag */           
    NVIC_ClearPendingIRQ(IICA1_IRQn);                    /* clear INTIICA interrupt flag */         
...
condition */
    ...
    NVIC_EnableIRQ(IICA1_IRQn);                          /* enable INTIICA interrupt flag */     
...
MD_STATUS IICA1_MasterSend(uint8_t adr, uint8_t * const tx_buf, uint16_t tx_num, uint8_t wait)
{
    ...
    NVIC_DisableIRQ(IICA1_IRQn);   /* disable INTIICA1 interrupt */

    if (IICA1->IICF1 & IICA1_IICF1_IICBSY_Msk)
    {
        /* Check bus busy */
        NVIC_EnableIRQ(IICA1_IRQn);   /* enable INTIICA1 interrupt */
        status = MD_ERROR1;
    }
    else
    {
